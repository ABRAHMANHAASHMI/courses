...
//    }
    ...
    public function index()
    {
        ...
        $data['subject_type'] = DB::table('subject_type')
            ->leftJoin('lesson', 'subject_type.id', '=', 'lesson.subject_type_id')
            ->select('subject_type.id as id', 'subject_type.name as name', 'subject_type.subject_id as subject_id', DB::raw("count(lesson.subject_type_id) as count"))
            ->groupBy('subject_type.id', 'subject_type.name', 'subject_type.subject_id')
            ->get();
...
/*        $data['lesson'] = DB::table('lesson')
            ...
            ->get();*/
        $data['lesson'] = Lesson::with('subject_type','lesson_img')
       // ->where('status', $user_info['plan_id'])
        ->get()->toArray();
        //echo '<pre>'; print_r( count($data['lesson'])); die;
        ...
        return view('home', compact('url'))->with($data);
    }
