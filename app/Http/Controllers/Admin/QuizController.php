...
class QuizController extends Controller
{
    ...
    public function add_quiz()
    {
        ...
        $lesson = Lesson::select()->where('type',2)->get()->toArray();
        $example = training_example::select()->get()->toArray();


        return view('admin/quiz/add_quiz', compact('example','lesson'));
    }
    ...
    public function add_quiz_type($type)
    {

        $lesson = Lesson::select()->where('type',2)->get()->toArray();
        $example = training_example::select()->get()->toArray();

        return view('admin/quiz/add_quiz_type', compact('example','lesson', 'type'));
    }

    public function all_quiz_type($type)
    {
        $status = $type == 'lesson' ? 1 : 2;
        $data['quiz'] = Quiz::where('status', $status)->with('answer')->get()->toArray();

        return view('admin/quiz/all_quiz_type', $data);
    }
    ...
    public function ax_update_quiz(Request $request){
    ...
    public function ax_delete_quiz(Request $request){
        $id = $request['id_quiz'];
        Quiz::select()->where('id',$id)->delete();
        return back();
    }
}
