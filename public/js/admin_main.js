...
if (action == 'AppHttpControllersAdminLessonController@add_lesson') {
    ...
    $(document).on('click', '.save_lesson', function () {
        ...
        var fields = {
            ...
            lesson_name: {errorname: 'Name', required: true},
            title: {errorname: 'Lesson title', required: true},
            lesson_video: {errorname: 'Lesson Video', nullabel: true},
        };

        if ($('#lesson_type').val() == 2) {

            fields.video_name = {errorname: 'Video', nullable: true};
        } else {
...
if (action == 'AppHttpControllersAdminQuizController@add_quiz' || action == 'AppHttpControllersAdminQuizController@add_quiz_type') {
    ...
    $(document).on('click', '.save_quiz', function () {
        var data = $('.quiz_form').serializeArray();

        /*        var add_steersman_fields  = {
         options:{selector:'.show_error', single:false, form_id:'.quiz_form'},
...
if (action == 'AppHttpControllersAdminLessonController@all_lesson') {
    ...
    $(document).on('click', '.show_lesson', function () {
        $('#show_lesson').modal('show');
        var id = $(this).attr('data-attr');
        $('.show_lesson_text_answer').html($(this).next().html())
    });
...
if (action == 'AppHttpControllersAdminQuizController@all_quiz' || action == 'AppHttpControllersAdminQuizController@all_quiz_type') {
